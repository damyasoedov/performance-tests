...
print('Open credit card account response: {}'.format(open_credit_card_account_response))
...
print('Make purchase operation response: {}'.format(make_purchase_operation_response))

get_accounts_request = GetAccountsRequest(
    user_id=create_user_response.user.id
)
get_accounts_response: GetAccountsResponse = \
    accounts_gateway_service.GetAccounts(get_accounts_request)
print('Get accounts response: {}'.format(get_accounts_response))
